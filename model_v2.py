	...
	def run_model(self):
		...
		for i in range(par['trials_per_seq']):
			...
			for k in range(par['num_time_steps']):
				...
				ctl_input = tf.concat([mask*self.stimulus_data[t], h_read, reward_matrix, action], axis = 1)
				...
				A = self.write_fast_weights(A, x, action, reward_matrix)
	...
	with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
		...
		for i in range(par['n_batches']):
			...
			_, reward, pol_loss, action, h = \
				...
				model.action, model.h], feed_dict=feed_dict)
			"""
			print( A.shape)
			print( probe.shape)
			...
			if i%20 == 0:
				print('Iter {:>4} | Reward: {:6.3f} | Pol. Loss: {:6.3f} | Stim Loss: {:6.6f}  | Mean h: {:6.6f}'.format(\
					i, reward, np.mean(pol_loss), np.mean(stim_loss), np.mean(h)))
