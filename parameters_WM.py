...
par = {
	...
	'dt'					: 100,
	'trials_per_seq'		: 50,
	'task_list'				: [a for a in range(1,141)], #[a for a in range(49)],
	'dead_trials'			: 40,
...
def update_dependencies():
	...
	par['W1_init'] = np.random.uniform(-0.02, 0.02, size=[par['n_ff0'], par['n_ff1'][0], par['n_ff1'][1]]).astype(np.float32)
	par['Wtd_init'] = np.random.uniform(-0.02, 0.02, size=[par['n_hidden'], par['n_ff0']]).astype(np.float32)
	par['b0_init'] = np.zeros([1,par['n_ff0']], dtype=np.float32)
	par['b1_init'] = np.zeros([1, par['n_ff1'][0], par['n_ff1'][1]], dtype=np.float32)

	# V0
	n_input_ctl = par['n_pol']*par['n_val']*par['n_modules'] + par['n_pol'] + par['n_val'] + par['n_input']
